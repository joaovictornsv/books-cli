...
name: gh-issue
description: >-
  Create GitHub issues for books-cli with consistent structure, branch naming,
...
5. Note the issue number from the URL. Update the body if it contains placeholders:
   - Branch name: `feat/<issue-number>-short-description` (use actual number)
   - PR close line: `Closes #<issue-number>`
...
Use kebab-case after the number. Example: `feat/24-add-wishlist-filter`.
...
- `docs/COMMANDS.md`, `README.md`, `.cursor/skills/books-cli/reference.md` — when commands or behavior change
...
7. Update `CHANGELOG.md`, `docs/COMMANDS.md`, `.cursor/skills/books-cli/reference.md`, and `README.md` as needed
...
10. **Release** (include when user-facing; per `.agents/github-releases/SKILL.md`):
    ...
    - Push `main` and tag
    - Build `books-linux-amd64`, generate `SHA256SUMS`, create GitHub release with concise bullet notes
    - Attach `books-linux-amd64` and `SHA256SUMS` to the release
```
...
- Imperative, concise: `Add wishlist filter to list command`
